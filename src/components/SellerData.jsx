    ...
    <div className='mx-auto max-w-7xl px-4 py-4'>
      ...
      <div className="mt-8 p-8 overflow-hidden text-md  rounded-lg shadow-lg bg-white border-gray-300 md:flex-row">
        ...
        <div className='text-end'>
          <button disabled={data.isDeleted} className={data.isDeleted ? `cursor-not-allowed bg-blue-900  px-5 py-2 text-white mx-2 rounded-md ` : ` bg-blue-900  px-5 py-2 text-white mx-2 rounded-md `} onClick={() => navigate(`/sellers/edit/${data?._id}`)}>Edit</button>
          <button className='border border-red-900 px-5 py-2 text-red-900 hover:bg-red-900 hover:text-white ease-in mx-2 rounded-md' onClick={handleDeleteSeller}>Delete</button>
        </div>
      </div>
    </div>
  )
